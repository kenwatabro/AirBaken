...
class Info:
    ...
    def get_track(self):
        ...
        driver.get(f'https://race.netkeiba.com/top/race_list.html?kaisai_date=' + str(self.year) + str(self.month).zfill(2) + str(self.day).zfill(2))

        time.sleep(5)  # Wait for the dynamic content to load

        # Find race elements on the page
        race_elements = driver.find_elements(By.CSS_SELECTOR, 'dl.RaceList_DataList')

        titles = []
        for race_element in race_elements:
            title_element = race_element.find_element(By.CSS_SELECTOR, 'dt.RaceList_DataHeader div.RaceList_DataHeader_Top p.RaceList_DataTitle')
        ...
        def get_URL(self, kai, place, day):
            ...
            kai = str(kai).zfill(2)
            day = str(day).zfill(2)
            self.race_url[place] = ['https://race.netkeiba.com/race/shutuba.html?race_id=' + str(self.year) + place_num + kai + day + str(i).zfill(2) for i in range(1, 13)]
...
#ラウンド
round_list = ['1R', '2R', '3R', '4R', '5R', '6R', '7R', '8R','9R', '10R', '11R', '12R']
            ...
            if bet_method.get() in ['単勝', '複勝']:
                if buying_method.get() != '通常':
                    ...
                    for string in buying_methods:
                        if string == '通常':
                            buy_menu["menu"].add_command(label=string, command=lambda value=string: buying_method.set(value))
            else:
                if buying_method.get() == '通常':
                    buy_menu["menu"].delete(0, "end")
                    for string in buying_methods:
...
# チェックボックスの状態を取得して選択された馬の名前を表示する新しいウィンドウを作成する関数
    def show_selected_horses(bet_method=bet_method, buying_method=buying_method):
        ...
        selected_horses = [df.iloc[i]['馬名'] for i in range(len(df)) if check_vars[i].get()]
        selected_horses_numbers = [df.iloc[i]['馬 番'] for i in range(len(df)) if check_vars[i].get()]

        # テキストとチェックボックスの列名を作成
        if bet_method.get() != '枠連':
            horse_number_label = tk.Label(selected_horses_window, text="馬番", anchor='w')
            horse_number_label.grid(row=0, column=0, sticky='w')
        elif bet_method.get() == '枠連':
            horse_number_label = tk.Label(selected_horses_window, text="枠番", anchor='w')
            horse_number_label.grid(row=0, column=0, sticky='w')
        horse_name_label = tk.Label(selected_horses_window, text="馬名", anchor='w')
        horse_name_label.grid(row=0, column=1, sticky='w')

        bet_method_dict = {'単勝':1, '複勝':1, 'ワイド':2, '馬連':2, '馬単':2, '枠連':2, '3連複':3, '3連単':3}
        ...
        if buying_method.get() == '通常':
            ...
            if bet_method.get() == '3連単':
                ...
                fixed_position_menu = tk.OptionMenu(selected_horses_window, fixed_position, *fixed_positions)
                fixed_position_menu.grid(row=len(selected_horses)+2, column=1)  # horse_name_labelの右に配置
            ...
            checkbox_labels = [f"{i}列目" for i in range(1, bet_method_dict[bet_method.get()]+1)]

        
        for i in range(len(checkbox_labels)):
            checkbox_label = tk.Label(selected_horses_window, text=checkbox_labels[i], anchor='w')
        ...
        for i, horse_number in enumerate(selected_horses_numbers):
            ...
            horse_number_label = tk.Label(selected_horses_window, text=str(horse_number), anchor='w')
            horse_number_label.grid(row=i+1, column=0, sticky='w')

            horse_name_label = tk.Label(selected_horses_window, text=horse_name, anchor='w')
            horse_name_label.grid(row=i+1, column=1, sticky='w')

            if buying_method.get() == '通常':
                var1 = tk.BooleanVar()
                check_button1 = tk.Checkbutton(selected_horses_window, variable=var1)
                check_button1.grid(row=i+1, column=2 + 0)
                check_vars1.append(var1)
                if bet_method_dict[bet_method.get()] >= 2:
                    var2 = tk.BooleanVar()
                    check_button2 = tk.Checkbutton(selected_horses_window, variable=var2)
                    check_button2.grid(row=i+1, column=2 + 1)
                    check_vars2.append(var2)
                    if bet_method_dict[bet_method.get()] >= 3:
                        var3 = tk.BooleanVar()
                        check_button3 = tk.Checkbutton(selected_horses_window, variable=var3)
        ...
        def show_all_bets(buying_method=buying_method, bet_method=bet_method, selected_horses_numbers=selected_horses_numbers, check_vars_all=check_vars_all):
            ...
            if bet_method.get() == '枠連':
                selected_horses_numbers = [waku[horse_number] for horse_number in selected_horses_numbers]
            def bet_normal(method, check_vars_all):
                num_of_horses = bet_method_dict[method]
                if num_of_horses==1:
                    check_vars_buy1 = check_vars_all[0]
                    horse_num_list = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_buy1[i].get()]
                    ...
                    horse_num_list1 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_buy1[i].get()]
                    horse_num_list2 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_buy2[i].get()]
                    ...
                    horse_num_list1 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_buy1[i].get()]
                    horse_num_list2 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_buy2[i].get()]
                    horse_num_list3 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_buy3[i].get()]
                    horse_num1 = horse_num_list1[0]
                    horse_num2 = horse_num_list2[0]
            ...
            def bet_box(method,  check_vars_all):
                ...
                if method in ['ワイド', '馬連', '枠連']:
                    patterns = [tuple(sorted([h1, h2])) for h1, h2 in combinations(horse_num_list, 2)]
                    
                elif method in ['馬単']:
                    patterns = [(h1, h2) for h1, h2 in permutations(horse_num_list, 2)]
                elif method in ['3連複']:
                    patterns = [tuple(sorted((h1, h2, h3))) for h1, h2, h3 in combinations(horse_num_list, 3)]
                    
                elif method in ['3連単']:
                    patterns = [(h1, h2, h3) for h1, h2, h3 in permutations(horse_num_list, 3)]
                return patterns
            
            def bet_nagashi(method, check_vars_all):
                check_vars_buy1 = check_vars_all[0]
                check_vars_buy2 = check_vars_all[1]
                jiku_num = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_buy1[i].get()][0]
                himo_list = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_buy2[i].get()]
                if method in ['ワイド', '馬連', '枠連']:
                    patterns = [tuple(sorted([jiku_num, himo])) for himo in himo_list if himo != jiku_num]
                    ...
                    patterns = [tuple(sorted((jiku_num, himo1, himo2))) for himo1, himo2 in combinations(himo_list, 2) if jiku_num != himo1 and jiku_num != himo2]
            ...
            def formation(method, check_vars_all):
                if method in ['馬連', 'ワイド', '枠連']:
                    horse_num_list1 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_all[0][i].get()]
                    horse_num_list2 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_all[1][i].get()]
                    patterns = [sorted([h1, h2]) for h1 in horse_num_list1 for h2 in horse_num_list2 if h1 != h2]
                
                elif method in ['馬単']:
                    horse_num_list1 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_all[0][i].get()]
                    horse_num_list2 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_all[1][i].get()]
                    patterns = [(h1, h2) for h1 in horse_num_list1 for h2 in horse_num_list2 if h1 != h2]

                elif method in ['3連複']:
                    horse_num_list1 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_all[0][i].get()]
                    horse_num_list2 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_all[1][i].get()]
                    horse_num_list3 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_all[2][i].get()]
                    patterns = [sorted([h1, h2, h3]) for h1 in horse_num_list1 for h2 in horse_num_list2 for h3 in horse_num_list3 if len({h1, h2, h3}) == 3]

                elif method in ['3連単']:
                    horse_num_list1 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_all[0][i].get()]
                    horse_num_list2 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_all[1][i].get()]
                    horse_num_list3 = [selected_horses_numbers[i] for i in range(len(selected_horses_numbers)) if check_vars_all[2][i].get()]
                    patterns = [(h1, h2, h3) for h1 in horse_num_list1 for h2 in horse_num_list2 for h3 in horse_num_list3 if len({h1, h2, h3}) == 3]
            ...
            for i, p in enumerate(patterns):
                index_label = tk.Label(all_bets_window, text=str(i+1), anchor='w')
                index_label.grid(row=i+1, column=0, sticky='w')
                pattern_label = tk.Label(all_bets_window, text=str(p), anchor='w')
            ...
            def record_csv(bet_method=bet_method, buying_method=buying_method, patterns=patterns, path='/air_baken.csv'):
                ...
                df_temp = pd.DataFrame(columns= ['date', '開催', 'R', '方式', '買い目', '購入金額', '1着', '2着', '3着', '払い戻し金額', 'race_id'])
            ...
            record_button = tk.Button(all_bets_window, text="Record", command=record_csv)
            record_button.grid(row=1, column=2, columnspan=2)

        show_all_bets_button = tk.Button(selected_horses_window, text="Show All Possible Bets", command=show_all_bets)
        show_all_bets_button.grid(row=len(selected_horses)+1, column=0, columnspan=2)

    show_selected_button = tk.Button(new_window, text="Show Selected Horses", command=show_selected_horses)
...
def return_calculate(path='/air_baken.csv'):
    ...
    for id in race_id_set:
        print(id)
        time.sleep(1)
        url = 'https://race.netkeiba.com/race/result.html?race_id=' + str(int(id))
        html = requests.get(url)
        html.encoding = "EUC-JP"
        first, second, third = pd.read_html(html.text)[0]['馬 番'][:3]
        df_result = pd.concat([pd.read_html(html.text)[1], pd.read_html(html.text)[2]])
        ...
        if '単勝' in df_result.index:
            ...
            tansho_res = [(int(x), int(y)) for x, y in zip(tansho_res_num, tansho_res_return)]
            res_dict['単勝'] = tansho_res

        if '複勝' in df_result.index:
            fukusho_res_num = df_result.loc['複勝', 1].split()
            fukusho_res_return = df_result.loc['複勝', 2].replace('円', '').split()
            for i in range(len(fukusho_res_return)):
                if ',' in fukusho_res_return[i]:
                    fukusho_res_return[i] = fukusho_res_return[i].replace(',', '')
            fukusho_res = [(int(x), int(y)) for x, y in zip(fukusho_res_num, fukusho_res_return)]
            res_dict['複勝'] = fukusho_res

        if '枠連' in df_result.index:
            wakuren_res_num = df_result.loc['枠連', 1].split()
            wakuren_res_return = df_result.loc['枠連', 2].replace('円', '').split()
            for i in range(len(wakuren_res_return)):
                if ',' in wakuren_res_return[i]:
                    wakuren_res_return[i] = wakuren_res_return[i].replace(',', '')
            wakuren_res_num = [(int(wakuren_res_num[i]), int(wakuren_res_num[i+1])) for i in range(0, len(wakuren_res_num), 2)]
            wakuren_res = [(x, int(y)) for x, y in zip(wakuren_res_num, wakuren_res_return)]
        ...
        if '馬連' in df_result.index:
            ...
            umaren_res_num = [(int(umaren_res_num[i]), int(umaren_res_num[i+1])) for i in range(0, len(umaren_res_num), 2)]
            umaren_res = [(x, int(y)) for x, y in zip(umaren_res_num, umaren_res_return)]
        ...
        if '馬単' in df_result.index:
            ...
            umatan_res_num = [(int(umatan_res_num[i]), int(umatan_res_num[i+1])) for i in range(0, len(umatan_res_num), 2)]
            umatan_res = [(x, int(y)) for x, y in zip(umatan_res_num, umatan_res_return)]
        ...
        if 'ワイド' in df_result.index:
            ...
            wide_res = [(tuple(sorted((int(x), int(y)))), int(z)) for (x, y), z in zip(wide_res_num, wide_res_return)]
            res_dict['ワイド'] = sorted(wide_res, key=lambda x: x[0])

        if '3連複' in df_result.index:
            sanrenpuku_res_num = df_result.loc['3連複', 1].split()
            sanrenpuku_res_return = df_result.loc['3連複', 2].replace('円', '').split()
            ...
            sanrenpuku_res_num = [(int(sanrenpuku_res_num[i]), int(sanrenpuku_res_num[i+1]), int(sanrenpuku_res_num[i+2])) for i in range(0, len(sanrenpuku_res_num), 3)]
            sanrenpuku_res = [(x, int(y)) for x, y in zip(sanrenpuku_res_num, sanrenpuku_res_return)]
            res_dict['3連複'] = sanrenpuku_res
        
        if '3連単' in df_result.index:
            sanrentan_res_num = df_result.loc['3連単', 1].split()
            sanrentan_res_return = df_result.loc['3連単', 2].replace('円', '').split()
            for i in range(len(sanrentan_res_return)):
                if ',' in sanrentan_res_return[i]:
                    sanrentan_res_return[i] = sanrentan_res_return[i].replace(',', '')
            sanrentan_res_num = [(int(sanrentan_res_num[i]), int(sanrentan_res_num[i+1]), int(sanrentan_res_num[i+2])) for i in range(0, len(sanrentan_res_num), 3)]
            sanrentan_res = [(x, int(y)) for x, y in zip(sanrentan_res_num, sanrentan_res_return)]
        ...
        for index, row in df_air_baken_temp.iterrows():
            ...
            if row['方式'] == '単勝':
                ...
                if kaime_tuple in atari:
                    ...
                    row['払い戻し金額'] = res_dict[row['方式']][payout_index][1]*row['購入金額']//100
                else:
                    row['払い戻し金額'] = 0
...
return_calculate_button = tk.Button(root, text="Return Calculate", command=return_calculate)
